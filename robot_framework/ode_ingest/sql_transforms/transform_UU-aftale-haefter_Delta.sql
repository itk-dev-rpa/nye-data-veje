    ...
    -- ============================================================
    -- Pipeline Script for UU-aftale-haefter (Delta)
    -- Source:  UU-aftale-haefter_Delta_Staging
    -- Targets: UU-aftale-haefter_Delta_Backup, UU-aftale-haefter_Delta_Typed, UU-aftale-haefter
    ...
    IF OBJECT_ID('[ode].[UU-aftale-haefter_Delta_Backup]', 'U') IS NULL
    BEGIN
        CREATE TABLE [ode].[UU-aftale-haefter_Delta_Backup] (
    ...
    IF OBJECT_ID('[ode].[UU-aftale-haefter_Delta_Typed]', 'U') IS NULL
    BEGIN
        CREATE TABLE [ode].[UU-aftale-haefter_Delta_Typed] (
    ...
    INSERT INTO [ode].[UU-aftale-haefter_Delta_Backup] (
        [Klient],
    [Aftale],
    [Aftalekonto],
    [Aftalenummer],
    ...
    [row_number],
    [etl_version]
    FROM [ode].[UU-aftale-haefter_Delta_Staging];
    ...
    INSERT INTO [ode].[UU-aftale-haefter_Delta_Typed] (
        [Klient],
    [Aftale],
    [Aftalekonto],
    [Aftalenummer],
    ...
    MERGE INTO [ode].[UU-aftale-haefter] AS target
    USING new_data AS source
    ON target.[Klient] = source.[Klient] AND target.[Aftalenummer] = source.[Aftalenummer] AND target.[Forretningspartner] = source.[Forretningspartner]
    WHEN MATCHED THEN
        UPDATE SET target.[Aftale] = source.[Aftale], target.[Aftalekonto] = source.[Aftalekonto], target.[BetFormReference] = source.[BetFormReference], target.[Hovedhæfter] = source.[Hovedhæfter], target.[Hæfter_fjernet] = source.[Hæfter_fjernet], target.[Hæfter_tilføjet] = source.[Hæfter_tilføjet], target.[Kommune_kode] = source.[Kommune_kode], target.[export_date] = source.[export_date], target.[file_origin] = source.[file_origin], target.[row_number] = source.[row_number], target.[etl_version] = source.[etl_version]
    WHEN NOT MATCHED THEN
        INSERT ([Klient], [Aftale], [Aftalekonto], [Aftalenummer], [BetFormReference], [Forretningspartner], [Hovedhæfter], [Hæfter_fjernet], [Hæfter_tilføjet], [Kommune_kode], [export_date], [file_origin], [row_number], [etl_version])
        VALUES (source.[Klient], source.[Aftale], source.[Aftalekonto], source.[Aftalenummer], source.[BetFormReference], source.[Forretningspartner], source.[Hovedhæfter], source.[Hæfter_fjernet], source.[Hæfter_tilføjet], source.[Kommune_kode], source.[export_date], source.[file_origin], source.[row_number], source.[etl_version]);
    ...
    SELECT 
        ...
        (SELECT COUNT(*) FROM [ode].[UU-aftale-haefter_Delta_Staging]) as source_rows,
    ...
    DROP TABLE [ode].[UU-aftale-haefter_Delta_Staging];
    GO
