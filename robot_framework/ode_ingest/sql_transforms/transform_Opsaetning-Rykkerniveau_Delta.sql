    ...
    -- ============================================================
    -- Pipeline Script for Opsaetning-Rykkerniveau (Delta)
    -- Source:  Opsaetning-Rykkerniveau_Delta_Staging
    -- Targets: Opsaetning-Rykkerniveau_Delta_Backup, Opsaetning-Rykkerniveau_Delta_Typed, Opsaetning-Rykkerniveau
    ...
    IF OBJECT_ID('[ode].[Opsaetning-Rykkerniveau_Delta_Backup]', 'U') IS NULL
    BEGIN
        CREATE TABLE [ode].[Opsaetning-Rykkerniveau_Delta_Backup] (
    ...
    IF OBJECT_ID('[ode].[Opsaetning-Rykkerniveau_Delta_Typed]', 'U') IS NULL
    BEGIN
        CREATE TABLE [ode].[Opsaetning-Rykkerniveau_Delta_Typed] (
    ...
    INSERT INTO [ode].[Opsaetning-Rykkerniveau_Delta_Backup] (
        [Sprognøgle],
    [Rykkeprocedure],
    [Rykkeniveau],
    [Betegnelse],
    ...
    [row_number],
    [etl_version]
    FROM [ode].[Opsaetning-Rykkerniveau_Delta_Staging];
    ...
    INSERT INTO [ode].[Opsaetning-Rykkerniveau_Delta_Typed] (
        [Sprognøgle],
    [Rykkeprocedure],
    [Rykkeniveau],
    [Betegnelse],
    ...
    MERGE INTO [ode].[Opsaetning-Rykkerniveau] AS target
    USING new_data AS source
    ON target.[Sprognøgle] = source.[Sprognøgle] AND target.[Rykkeprocedure] = source.[Rykkeprocedure] AND target.[Rykkeniveau] = source.[Rykkeniveau]
    WHEN MATCHED THEN
        UPDATE SET target.[Betegnelse] = source.[Betegnelse], target.[Kommune_kode] = source.[Kommune_kode], target.[export_date] = source.[export_date], target.[file_origin] = source.[file_origin], target.[row_number] = source.[row_number], target.[etl_version] = source.[etl_version]
    WHEN NOT MATCHED THEN
        INSERT ([Sprognøgle], [Rykkeprocedure], [Rykkeniveau], [Betegnelse], [Kommune_kode], [export_date], [file_origin], [row_number], [etl_version])
        VALUES (source.[Sprognøgle], source.[Rykkeprocedure], source.[Rykkeniveau], source.[Betegnelse], source.[Kommune_kode], source.[export_date], source.[file_origin], source.[row_number], source.[etl_version]);
    ...
    SELECT 
        ...
        (SELECT COUNT(*) FROM [ode].[Opsaetning-Rykkerniveau_Delta_Staging]) as source_rows,
    ...
    DROP TABLE [ode].[Opsaetning-Rykkerniveau_Delta_Staging];
    GO
