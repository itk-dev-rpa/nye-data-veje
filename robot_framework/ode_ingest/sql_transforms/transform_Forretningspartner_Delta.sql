    ...
    -- ============================================================
    -- Pipeline Script for Forretningspartner (Delta)
    -- Source:  Forretningspartner_Delta_Staging
    -- Targets: Forretningspartner_Delta_Backup, Forretningspartner_Delta_Typed, Forretningspartner
    ...
    IF OBJECT_ID('[ode].[Forretningspartner_Delta_Backup]', 'U') IS NULL
    BEGIN
        CREATE TABLE [ode].[Forretningspartner_Delta_Backup] (
    ...
    IF OBJECT_ID('[ode].[Forretningspartner_Delta_Typed]', 'U') IS NULL
    BEGIN
        CREATE TABLE [ode].[Forretningspartner_Delta_Typed] (
    ...
    INSERT INTO [ode].[Forretningspartner_Delta_Backup] (
        [Klient],
    [Forretningspartner],
    [Identifikationsart],
    [Gyldig_fra],
    ...
    [row_number],
    [etl_version]
    FROM [ode].[Forretningspartner_Delta_Staging];
    ...
    INSERT INTO [ode].[Forretningspartner_Delta_Typed] (
        [Klient],
    [Forretningspartner],
    [Identifikationsart],
    [Gyldig_fra],
    ...
    MERGE INTO [ode].[Forretningspartner] AS target
    USING new_data AS source
    ON target.[Klient] = source.[Klient] AND target.[Forretningspartner] = source.[Forretningspartner] AND target.[Identifikationsart] = source.[Identifikationsart] AND target.[Forretningspartner-GUID] = source.[Forretningspartner-GUID]
    WHEN MATCHED THEN
        UPDATE SET target.[Gyldig_fra] = source.[Gyldig_fra], target.[Gyldig_til] = source.[Gyldig_til], target.[Datafordeleren_UUID] = source.[Datafordeleren_UUID], target.[BW_Opdateringsmode] = source.[BW_Opdateringsmode], target.[Slettet] = source.[Slettet], target.[export_date] = source.[export_date], target.[file_origin] = source.[file_origin], target.[row_number] = source.[row_number], target.[etl_version] = source.[etl_version]
    WHEN NOT MATCHED THEN
        INSERT ([Klient], [Forretningspartner], [Identifikationsart], [Gyldig_fra], [Gyldig_til], [Datafordeleren_UUID], [Forretningspartner-GUID], [BW_Opdateringsmode], [Slettet], [export_date], [file_origin], [row_number], [etl_version])
        VALUES (source.[Klient], source.[Forretningspartner], source.[Identifikationsart], source.[Gyldig_fra], source.[Gyldig_til], source.[Datafordeleren_UUID], source.[Forretningspartner-GUID], source.[BW_Opdateringsmode], source.[Slettet], source.[export_date], source.[file_origin], source.[row_number], source.[etl_version]);
    ...
    SELECT 
        ...
        (SELECT COUNT(*) FROM [ode].[Forretningspartner_Delta_Staging]) as source_rows,
    ...
    DROP TABLE [ode].[Forretningspartner_Delta_Staging];
    GO
