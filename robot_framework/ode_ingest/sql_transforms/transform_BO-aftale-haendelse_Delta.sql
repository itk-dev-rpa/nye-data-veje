    ...
    -- ============================================================
    -- Pipeline Script for BO-aftale-haendelse (Delta)
    -- Source:  BO-aftale-haendelse_Delta_Staging
    -- Targets: BO-aftale-haendelse_Delta_Backup, BO-aftale-haendelse_Delta_Typed, BO-aftale-haendelse
    ...
    IF OBJECT_ID('[ode].[BO-aftale-haendelse_Delta_Backup]', 'U') IS NULL
    BEGIN
        CREATE TABLE [ode].[BO-aftale-haendelse_Delta_Backup] (
    ...
    IF OBJECT_ID('[ode].[BO-aftale-haendelse_Delta_Typed]', 'U') IS NULL
    BEGIN
        CREATE TABLE [ode].[BO-aftale-haendelse_Delta_Typed] (
    ...
    INSERT INTO [ode].[BO-aftale-haendelse_Delta_Backup] (
        [Klient],
    [Aftalenummer],
    [Bobehandling],
    ...
    [row_number],
    [etl_version]
    FROM [ode].[BO-aftale-haendelse_Delta_Staging];
    ...
    INSERT INTO [ode].[BO-aftale-haendelse_Delta_Typed] (
        [Klient],
    [Aftalenummer],
    [Bobehandling],
    [Ekstern_reference_nøgle],
    ...
    MERGE INTO [ode].[BO-aftale-haendelse] AS target
    USING new_data AS source
    ON target.[Ekstern_reference_nøgle] = source.[Ekstern_reference_nøgle]
    WHEN MATCHED THEN
        UPDATE SET target.[Klient] = source.[Klient], target.[Aftalenummer] = source.[Aftalenummer], target.[Bobehandling] = source.[Bobehandling], target.[Udtrækstype] = source.[Udtrækstype], target.[Korrespondanceart] = source.[Korrespondanceart], target.[Printdato] = source.[Printdato], target.[Afskrevet] = source.[Afskrevet], target.[Afskrivnings_dato] = source.[Afskrivnings_dato], target.[Betalingsdato] = source.[Betalingsdato], target.[Valuta] = source.[Valuta], target.[Kommune_kode] = source.[Kommune_kode], target.[Beløb] = source.[Beløb], target.[Indbetalt_af_bobestyrer] = source.[Indbetalt_af_bobestyrer], target.[export_date] = source.[export_date], target.[file_origin] = source.[file_origin], target.[row_number] = source.[row_number], target.[etl_version] = source.[etl_version]
    WHEN NOT MATCHED THEN
        INSERT ([Klient], [Aftalenummer], [Bobehandling], [Ekstern_reference_nøgle], [Udtrækstype], [Korrespondanceart], [Printdato], [Afskrevet], [Afskrivnings_dato], [Betalingsdato], [Valuta], [Kommune_kode], [Beløb], [Indbetalt_af_bobestyrer], [export_date], [file_origin], [row_number], [etl_version])
        VALUES (source.[Klient], source.[Aftalenummer], source.[Bobehandling], source.[Ekstern_reference_nøgle], source.[Udtrækstype], source.[Korrespondanceart], source.[Printdato], source.[Afskrevet], source.[Afskrivnings_dato], source.[Betalingsdato], source.[Valuta], source.[Kommune_kode], source.[Beløb], source.[Indbetalt_af_bobestyrer], source.[export_date], source.[file_origin], source.[row_number], source.[etl_version]);
    ...
    SELECT 
        ...
        (SELECT COUNT(*) FROM [ode].[BO-aftale-haendelse_Delta_Staging]) as source_rows,
    ...
    DROP TABLE [ode].[BO-aftale-haendelse_Delta_Staging];
    GO
